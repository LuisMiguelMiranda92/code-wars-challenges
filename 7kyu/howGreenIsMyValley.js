...
function makeValley(arr) {
    ...
    while (sortedArray.length > 0) {
        if (sortedArray.length % 2 !== 0) {
            leftWing.push(sortedArray.shift()); //Use shift instead of pop, to get alternate elements from beginning.
        } else{
            rightWing.push(sortedArray.shift()); 

        }
        }
    return leftWing.reverse().concat(rightWing); //Reverse the right wing and concat with left wing.
}