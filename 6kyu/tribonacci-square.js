...
function tribonacci(signature,n){
    ...
    //while signature.length is less than n
    while(signature.length < n){
    //sum the last 3 numbers of signature and push it
    let lastThree = signature.slice(-3)
    let result = lastThree.reduce((sum, currentValue) => sum + currentValue, 0)
    signature.push(result)
    }
    //once finished return signature
    return signature
    }
  }
